...
// Cargar preguntas: se cargan desde window.PREGUNTAS_GLOBALES (js/questions.js)
async function importarPreguntasDeGlobal() {
    console.log("🔄 main.js: Intentando importar preguntas de window.PREGUNTAS_GLOBALES...");
    try {
        // Debug: Ver qué hay en la global
        console.log("🔍 Contenido de window.PREGUNTAS_GLOBALES:", JSON.stringify(window.PREGUNTAS_GLOBALES, (key, value) => (key === 'opciones' ? `[${value.length}]` : value)).slice(0, 100) + "...");

        if (typeof window.PREGUNTAS_GLOBALES !== 'undefined' && window.PREGUNTAS_GLOBALES) {
            // Verificar si tiene claves
            const keys = Object.keys(window.PREGUNTAS_GLOBALES);
            if (keys.length === 0) {
                console.warn("⚠️ window.PREGUNTAS_GLOBALES existe pero está vacío.");
            }

            baseDatos = window.PREGUNTAS_GLOBALES;
            console.log('✓ Preguntas importadas exitosamente a baseDatos local.');
            console.log(`  - Básico: ${baseDatos.basico?.length || 0}`);
            console.log(`  - Intermedio: ${baseDatos.intermedio?.length || 0}`);
            console.log(`  - Avanzado: ${baseDatos.avanzado?.length || 0}`);
            return;
        }

        throw new Error('window.PREGUNTAS_GLOBALES no está definida.');
    } catch (error) {
...
class GameController {
    constructor() {
        ...
        this.timerInterval = null; // Variable para el intervalo del cronómetro
    ...
    inicializarElementos() {
        ...
        this.elementos = {
            ...
            timerDisplay: document.getElementById('quiz-timer-text'), // ID actualizado
            ...
            timeFinal: document.getElementById('time-final'), // Tiempo total en resultados
            ...
            const updateAutoAdvUI = () => {
                ...
                if (this.autoAdvMsg) {
                    ...
                    if (this.autoAdvanceEnabled) {
                        this.autoAdvMsg.innerHTML = "🔁 <b>Activado:</b> Avanza automáticamente";
                        this.autoAdvMsg.className = "text-primary small d-block mt-2";
                    } else {
                        this.autoAdvMsg.innerHTML = "⏸️ <b>Desactivado:</b> Avance manual (clic en Continuar)";
                        this.autoAdvMsg.className = "text-muted small d-block mt-2";
                    }
                }
            };
    ...
    iniciarJuego() {
        // 1. INICIAR RELOJ PRIMERO (Prioridad absoluta)
        console.log("🎮 :: iniciarJuego llamando a reloj...");
        if (typeof window.iniciarTimerQuiz === 'function') {
            window.iniciarTimerQuiz();
        } else {
            console.error("❌ Función iniciarTimerQuiz no encontrada en window");
        }
        ...
        if (this.autoAdvMsg) this.autoAdvMsg.classList.remove('d-none');
        if (this.continueBtn) this.continueBtn.classList.add('d-none');

        // Mantener compatibilidad con métricas internas (opcional)
        // this._startTimerRobust(); 

        this.mostrarPregunta();
    }

    _stopTimer() {
        if (typeof window.detenerTimerQuiz === 'function') {
    ...
    mostrarPregunta() {
        ...
        // Normalizar tipo a Sentence case (primera mayúscula, resto minúscula)
        const tipoTexto = pregunta.tipo || '';
        const tipoNormalizado = tipoTexto.charAt(0).toUpperCase() + tipoTexto.slice(1).toLowerCase();
        this.elementos.questionType.textContent = '✍️ ' + tipoNormalizado;
    ...
    finalizarJuego() {
        // 1. Detener Cronómetro (global o local)
        this._stopTimer();

        // 2. Capturar el tiempo (USAR LA FUENTE DE VERDAD)
        let timeString = "00:00";
        if (typeof window.obtenerTiempoFormateado === 'function') {
            timeString = window.obtenerTiempoFormateado();
        } else {
            // Fallback al DOM si la función no existe (por seguridad)
            const visualTimer = document.getElementById('quiz-timer-text');
            if (visualTimer && visualTimer.innerText) {
                timeString = visualTimer.innerText;
            }
        }
        ...
        // Asignar el tiempo capturado
        if (this.elementos.timeFinal) {
            let finalTime = "00:00";
            // Intentar obtener tiempo formateado de la fuente de verdad
            if (typeof window.obtenerTiempoFormateado === 'function') {
                finalTime = window.obtenerTiempoFormateado();
            } else {
                // Fallback: Capturar del DOM como pidió el usuario
                const timerDisplay = document.getElementById('quiz-timer-text');
                if (timerDisplay) finalTime = timerDisplay.innerText;
            }
            this.elementos.timeFinal.textContent = finalTime;
        }
        ...
        // Calcular tiempo preciso en segundos
        let durationSec = "00.00";
        if (this.stats && this.stats.tiempoInicio) {
            const endTime = Date.now();
            const durationMs = endTime - this.stats.tiempoInicio;
            durationSec = (durationMs / 1000).toFixed(2); // Ej: "14.50"
        }

        // 2.5 GUARDAR EN LA NUBE (SaaS)
        if (typeof window.guardarPuntajeCloud === 'function') {
            console.log("☁️ Iniciando guardado en nube...");
            window.guardarPuntajeCloud({
                nivel: this.nivelActual,
                puntaje: this.stats.aciertos,
                total: this.preguntas.length,
                tiempo: timeString, // "MM:SS" (Visual Legacy)
                tiempoExacto: durationSec // "SS.ss" (Nuevo Ranking Preciso)
            });
        }

        // 3. Efecto Celebración (Confeti) si es puntuación perfecta
        if (this.stats.obtenerPorcentaje() === 100) {
            if (typeof lanzarConfeti === 'function') {
                setTimeout(() => lanzarConfeti(), 500); // Pequeño delay para que coincida con la pantalla
            }
        }
    ...
    cambiarPantalla(pantalla) {
        ...
        // Logic for reducing app-container height on results screen
        const appContainer = document.querySelector('.app-container');
        if (appContainer) {
            if (pantalla === 'result') {
                appContainer.classList.add('app-short-height');
            } else {
    ...
    salir() {
        this._stopTimer(); // Asegurar detener timer al salir
        this.cambiarPantalla('start');
    }

    volverInicio() {
        this._stopTimer(); // Asegurar detener timer al volver al inicio
...
async function initializeGame() {
    ...
    // Importar preguntas
    await importarPreguntasDeGlobal();

    // Validar contenido real
    const total = (baseDatos.basico?.length || 0) + (baseDatos.intermedio?.length || 0) + (baseDatos.avanzado?.length || 0);
    console.log(`📊 Base de Datos local: ${total} preguntas encontradas.`);

    if (total === 0) {
        console.error("❌ ERROR: No hay preguntas cargadas. Verifica tu conexión o Firebase.");
        alert("Error: No se pudieron descargar las preguntas del servidor de Atiy Pukllay.");
        return false;
    }
    ...
    if (!game) {
        try {
            ...
            console.log("✅ GameController creado exitosamente.");
        } catch (e) {
            console.error("❌ Error al crear GameController:", e);
        }
    }
    return true;
}
...
// ============================================
// GESTIÓN DE SESIÓN DE ALUMNO
// ============================================

document.addEventListener('DOMContentLoaded', () => {
    // Intentar actualizar saludo si existen los elementos
    const nameEl = document.getElementById('nav-user-name');
    if (nameEl) actualizarSaludoUsuario();
});

function actualizarSaludoUsuario() {
    const userName = localStorage.getItem('atiy_student_name');
    const userSection = localStorage.getItem('atiy_student_section');

    const nameEl = document.getElementById('nav-user-name');
    const sectionEl = document.getElementById('nav-user-section');
    ...
    if (userSection && sectionEl) {
        sectionEl.textContent = `Salón: ${userSection}`;
    }
}

// Función global de cierre de sesión
// Función global de cierre de sesión mejorada
window.cerrarSesion = function () {
    const doLogout = () => {
        localStorage.removeItem('atiy_key');
